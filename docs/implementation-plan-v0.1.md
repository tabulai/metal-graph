...
*Prepared 2026-07-10; rev B 2026-07-10 incorporating external technical review. Companion to `apple-gpu-graph-acceleration.md` (the cuGraph assessment); section references like "Part 2.2" point there. cuGraph file references are to the checkout at `cuGraph_assessment/cugraph` @ `f66567f4`.*
...
The selection criterion is *acceleration ceiling on this hardware*: these are pure memory-bandwidth-bound workloads whose inner loops map 1:1 onto Metal capabilities with **no sorts, no hash maps, and no fp64 in the iteration path** — precisely avoiding the places Metal makes you pay (Part 2.2) while leaning on the 273–819 GB/s of unified-memory bandwidth that CPUs on the same machine cannot touch on random access. They need only 32-bit integer atomics, `atomic<float>`, SIMD-group reductions/prefix sums, and indirect dispatch — all present on every Apple Silicon Mac (Metal 3, Apple7+). And they are the top of the Part 4.3 value ranking: batched PPR *is* the HippoRAG/aider query path, k-hop *is* Graphiti/LightRAG retrieval, WCC backs entity dedup.
...
**Why worklists, not permutation ranges** (review item 3): a single degree-sorted ID permutation cannot make high/mid/low ranges contiguous for *both* orientations of a directed graph — in-degree and out-degree orders differ, and adversarial graphs make them disjoint. So segmentation is decoupled from identity: canonical IDs are fixed once, and each orientation carries three vertex worklists binned by *that orientation's* degree (thresholds 1024/32 per Part 1.3, retuned in M4). Kernels read `v = worklist[wi]` — one extra 4-byte streaming read per vertex, negligible against 8 B/edge. cuGraph's hypersparse/DCSR fourth segment remains dropped (multi-GPU artifact; Part 1.3).

**Build pipeline (CPU, threaded)**: external-ID map → degree histograms (both directions) → canonical order by out-degree (stable) → counting-sort edges into CSR → emit worklists, zero-fill lists, dangling list, `out_weight_sum`. All buffers allocated shared-mode up front — the build *is* the upload. Throughput target is **measured in M1, not asserted**: the harness times build separately (§10) and the target is set from the first real measurement (the assessment's ≥100M edges/s estimate is provisional until then).
...
2. **Engineering targets** (expected, not ship-blocking, from the Part 5.4 model): PageRank ≤6 ms/iteration on soc-LiveJournal1 on M4 Pro (≥36% BW; stretch ≤4 ms); 30-iteration run ≥15× igraph. BFS LiveJournal ≤50 ms (stretch ≤20 ms). WCC ≤5× BFS time.
